...
class Edge
{
  public:
    ...
    Edge(int pixelIdxA, int pixelIdxB, int cost) : pixelIdxA(pixelIdxA), pixelIdxB(pixelIdxB), cost(cost)
    {
    }
    ...
    static void
    calcEdges(float theta0, int x, int y, const cv::Mat& theta, const cv::Mat& mag, std::vector<Edge>& edges);

    //! Process edges in order of increasing cost, merging clusters if we can do so without exceeding the thetaThresh.
    static void
    mergeEdges(std::vector<Edge>& edges, UnionFindSimple& uf, float tmin[], float tmax[], float mmin[], float mmax[]);
};
