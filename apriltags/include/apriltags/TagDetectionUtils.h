...
void extract_edges(const cv::Mat& fimSeg, const cv::Mat& fimMag, const cv::Mat& fimTheta, UnionFindSimple& uf);

/*! \brief Step 4-5. Fit line segments */
void fit_segments(const cv::Mat& fimSeg,
                  const cv::Mat& fimMag,
...
std::vector<TagDetection> decode_quads(const cv::Mat& fim, const std::vector<Quad>& quads, const TagFamily& family);
